...
import { updateTestCentre } from "../../store/actions";

const UpdateTestingCentre = ({ isOpen, handleClose }) => {
  ...
  const { isUpdatingTestCentre, isGettingTestCentreById, testCentreById } =
    useSelector((state) => state.covidTest);

  useEffect(() => {
    if (testCentreById) {
      setTestCentreId(testCentreById?.testCentreId);
      setCenterName(testCentreById?.centreName);
      setDistrict(testCentreById?.district);

      let dsDList = DS_DIVISION_LIST.find(
        (x) => x.key === testCentreById?.district
      )?.dsDivisions;
      setDsDivisionsList(dsDList);

      setDsDivision(testCentreById?.city);
      setAddress(testCentreById?.address);
      setPhoneNumber(testCentreById?.phoneNumber);
      setLatitude(testCentreById?.latitude);
      setLongitude(testCentreById?.longitude);
      setStatus(testCentreById?.isActive ? "Active" : "Inactive");
  ...
  const handleUpdate = (e) => {
    ...
    const userData = {
      ...
      createdDate: testCentreById?.createdDate,
      createdBy: testCentreById?.createdBy,
    };

    dispatch(updateTestCentre(userData, handleClose));
  };
        ...
        <FormWrapper onSubmit={checkValidity}>
          <Grid container spacing={2}>
            <Grid item sm={12} xs={12}>
              <Heading title={"Update Testing Center"} />
            </Grid>
            <Grid item sm={12} xs={12}>
              <TextField
                disabled
...
export default UpdateTestingCentre;

...
const CreateVaccinationAppointment = ({ isOpen, handleClose }) => {
  ...
  const { isCreatingVaccineAppointment } = useSelector(
      ...
      <FormWrapper onSubmit={checkValidity}>
        <Grid container spacing={2}>
          <Grid item sm={12} xs={12}>
            <Heading title={"Create Vaccination Appointment"} />
          </Grid>
          <Grid item sm={12} xs={12}>
            <TextField
              required
        ...
        {!isValidCentre && (
          ...
          >
            * Select a vaccination center to continue
          </Typography>
        )}
        <FormActionButton
          onCancelClick={onHandleCancel}
          loading={isCreatingVaccineAppointment}
        />
      </FormWrapper>
    </CustomModal>
  );
...
export default CreateVaccinationAppointment;

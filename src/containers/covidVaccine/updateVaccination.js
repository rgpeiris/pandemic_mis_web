...
import { updatePandemicVaccination } from "../../store/actions";

const UpdateVaccination = ({ isOpen, handleClose }) => {
  const dispatch = useDispatch();

  const [pandemicVaccinationId, setPandemicVaccinationId] = useState("");
  ...
  const {
    covidVaccines,
    isGettingVaccines,
    isUpdatingPandemicVaccination,
    vaccinationCentres,
    isGettingVaccinationCentres,
    isGettingPandemicVaccinationById,
    pandemicVaccinationById,
  } = useSelector((state) => state.covidVaccine);

  useEffect(() => {
    if (pandemicVaccinationById) {
      setPandemicVaccinationId(pandemicVaccinationById?.pandemicVaccinationId);
      setFullName(pandemicVaccinationById?.name);
      setNicNumber(pandemicVaccinationById?.nicNumber);
      setAge(pandemicVaccinationById?.age);
      setGender(pandemicVaccinationById?.gender);
      setPhoneNumber(pandemicVaccinationById?.phoneNumber);
      setAddress(pandemicVaccinationById?.address);
      setDistrict(pandemicVaccinationById?.district);

      let moList = MOH_LIST.find(
        (x) => x.key === pandemicVaccinationById?.district
      )?.mohList;
      setMohList(moList);

      setMoh(pandemicVaccinationById?.mohArea);
      ...
      setGnd(pandemicVaccinationById?.gramaNiladhariArea);

      setVaccineName(pandemicVaccinationById?.vaccineName);
      setVaccineDose(pandemicVaccinationById?.vaccineDose);
      setCenter(pandemicVaccinationById?.vaccinationCentreId);

      let cen = vaccinationCentres.find(
        (x) =>
          x.vaccinationCentreId === pandemicVaccinationById?.vaccinationCentreId
      );
      setSelectedCenter(cen);
      setDateOfVaccination(
        pandemicVaccinationById?.dateOfVaccination?.split("T")[0]
      );
      setNextAppointmentDate(pandemicVaccinationById?.nextAppointmentDate);
      setSerialNumber(pandemicVaccinationById?.serialNumber);
  ...
  const onHandleVaccineName = (e) => {
    ...
    if (vaccineName && vaccineDose && dateOfVaccination) {
  ...
  const handleUpdate = (e) => {
    ...
    const userData = {
      ...
      dateOfVaccination,
      nextAppointmentDate,
      createdDate: pandemicVaccinationById?.createdDate,
      createdBy: pandemicVaccinationById?.createdBy,
    };

    dispatch(updatePandemicVaccination(userData, handleClose));
  };
        ...
        <FormWrapper onSubmit={checkValidity}>
          <Grid container spacing={2}>
            <Grid item sm={12} xs={12}>
              <Heading title={"Update Vaccination"} />
            </Grid>
            <Grid item sm={12} xs={12}>
              <TextField
                required
            ...
            <Grid item sm={6} xs={12}>
              <CustomSelect
                ...
                selectData={
                  covidVaccines.filter((obj, index) => {
                    return (
                      index ===
          ...
          <FormActionButton
            ...
            loading={isUpdatingPandemicVaccination}
          />
        </FormWrapper>
      )}
    </CustomModal>
